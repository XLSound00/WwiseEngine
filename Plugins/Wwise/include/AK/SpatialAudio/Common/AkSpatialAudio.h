...
struct AkSpatialAudioInitSettings
{
	...
	AkUInt32 uMaxDiffractionOrder;					///< Maximum diffraction order [1, 8] - the number of 'bends' in a diffraction path. A higher diffraction order accommodates more complex geometry at the expense of higher CPU usage.
	...
	AkUInt32 uDiffractionOnReflectionsOrder;		///< The maximum possible number of diffraction points at each end of a reflection path. Diffraction on reflection allows reflections to fade in and out smoothly as the listener or emitter move in and out of the reflection's shadow zone.
													///< When greater than zero, diffraction rays are fired from the listener to search for reflections around one or more corners from the listener.
