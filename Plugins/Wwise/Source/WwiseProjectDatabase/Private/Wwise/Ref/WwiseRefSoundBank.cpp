...
WwiseMediaIdsMap FWwiseRefSoundBank::GetMedia(const WwiseMediaGlobalIdsMap& GlobalMap) const
{
...
WwiseCustomPluginIdsMap FWwiseRefSoundBank::GetCustomPlugins(const WwiseCustomPluginGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
...
WwiseAudioDeviceIdsMap FWwiseRefSoundBank::GetAudioDevices(const WwiseAudioDeviceGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank || !SoundBank->Plugins)
	{
...
WwiseEventIdsMap FWwiseRefSoundBank::GetEvents(const WwiseEventGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseDialogueEventIdsMap FWwiseRefSoundBank::GetDialogueEvents(const WwiseDialogueEventGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseDialogueArgumentIdsMap FWwiseRefSoundBank::GetAllDialogueArguments(const WwiseDialogueArgumentGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseBusIdsMap FWwiseRefSoundBank::GetBusses(const WwiseBusGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseAuxBusIdsMap FWwiseRefSoundBank::GetAuxBusses(const WwiseAuxBusGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseGameParameterIdsMap FWwiseRefSoundBank::GetGameParameters(const WwiseGameParameterGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseStateGroupIdsMap FWwiseRefSoundBank::GetStateGroups(const WwiseStateGroupGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseStateIdsMap FWwiseRefSoundBank::GetAllStates(const WwiseStateGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseSwitchGroupIdsMap FWwiseRefSoundBank::GetSwitchGroups(const WwiseSwitchGroupGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseSwitchIdsMap FWwiseRefSoundBank::GetAllSwitches(const WwiseSwitchGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseTriggerIdsMap FWwiseRefSoundBank::GetTriggers(const WwiseTriggerGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseExternalSourceIdsMap FWwiseRefSoundBank::GetExternalSources(const WwiseExternalSourceGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
...
WwiseAcousticTextureIdsMap FWwiseRefSoundBank::GetAcousticTextures(const WwiseAcousticTextureGlobalIdsMap& GlobalMap) const
{
	const auto* SoundBank = GetSoundBank();
	if (!SoundBank)
	{
